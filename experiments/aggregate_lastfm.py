#!/usr/bin/env python3
"""Aggregate Last.fm-2k logs across seeds (mirrors aggregate_movielens.py)."""
...
OUT_MD = ROOT / "experiments" / "results" / "lastfm_table.md"
...
def main():
    ...
    out = []
    out.append("# Resultados — Last.fm-2k (top 1500 artistas)")
    out.append("")
    out.append(f"Média sobre {len(runs)} seeds. Test split: 90 dias.")
    out.append("")
    out.append("## Sistema vs baselines (Last.fm-2k, cold start sintético)")
    out.append("")
    out.append("| Modelo | P@10 | NDCG@10 | Coverage | Multiplicador |")
    out.append("|---|---|---|---|---|")
    ...
    for bname, label in [
        ...
        ndcg = agg.get(f"{bname}_ndcg10", {"n": 0, "mean": 0, "std": 0})
    ...
    out.append("## Wilcoxon signed-rank (P@10)")
    out.append("")
    out.append("| Baseline | p-value (média) | Win-rate | Significativo? |")
    out.append("|---|---|---|---|")
    for bname, label in [
        ("random", "Aleatório"),
        ("pop", "Popularidade"),
